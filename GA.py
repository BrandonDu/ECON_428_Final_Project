...
def build_lstm_model(hyperparameters, input_shape):
    model = Sequential()
...
def fitness_function(hyperparameters, X_train, y_train, X_val, y_val):
    model = build_lstm_model(hyperparameters, input_shape=(X_train.shape[1], X_train.shape[2]))
...
def selection(population, fitnesses, num_parents):
    parents = random.choices(population, weights=fitnesses, k=num_parents) # selects num_parents individuals with probability proportional to fitness score
    return parents
