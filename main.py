...
from LSTM import evaluate_lstm_model, create_sequences
...
def ARO(F_index, MaxIt, nPop, train_data, test_data):
    # Define function range
    Low, Up, Dim = fun_range(F_index)

    # Initialize population
    PopPos = np.random.rand(nPop, Dim) * (
            Up - Low) + Low  # nPop number of vectors of dimension "Dim" with lower and upper bounds defined
    PopFit = np.zeros(nPop)

    # Evaluate initial population
    for i in range(nPop):
        # PopFit[i] = ben_functions(PopPos[i, :], F_index, Dim)
        PopFit[i], _ = evaluate_lstm_model(PopPos[i, :], train_data, test_data)
    ...
    for It in range(MaxIt):
        ...
        for i in range(nPop):
            ...
            if len(newPopPos.shape) == 2 and newPopPos.shape[0] == 1:
                newPopPos = newPopPos.reshape(newPopPos.shape[1])
            # newPopFit = ben_functions(newPopPos, F_index, Dim)
            newPopFit, _ = evaluate_lstm_model(newPopPos, train_data, test_data)
...
best_hyperparams, best_error, history = ARO(FunIndex, MaxIteration, PopSize, training_data, test_data)
print(f"best_hyperparams = {best_hyperparams}")
print(f"best_error = {best_error}")
print(f"history = {history}")

mse, best_model = evaluate_lstm_model(best_hyperparams, training_data, test_data,
                                      graphs=True)  # Evaluate the best hyperparameters, show the graph
print(f"mse={mse}")

print(f'The best MSE of F{FunIndex} is: {best_error}')
...
scaled_data = scaled_data.reshape((scaled_data.shape[1], scaled_data.shape[0], len(sp500_stocks)))
print(scaled_data.shape)

predictions = best_model.predict(scaled_data)  # predict on the normalized data
predicted_prices = scaler.inverse_transform(predictions)
predicted_price = predicted_prices[-1][0]
print(predicted_prices)

if predicted_price > stock_data['Open'][-1]:
    print("Long")
else:
    print("Short")
